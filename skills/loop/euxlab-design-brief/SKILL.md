---
name: euxlab-design-brief
description: Turn cited insights into the design assignment - scope, requirements, and the specific insights each requirement serves - agreed with the stakeholder before any design exists. Run after /euxlab-synthesize, before any design work.
...
Load `/euxlab-outcome-vocabulary` for the outcome grammar and `/euxlab-evidence-standards` for grading. No interview at the drafting stage; this is synthesis. The interview happens at step 5, with the stakeholder.
...
Each cluster becomes one requirement. Five to nine requirements for an engagement. A requirement list that covers everything is a backlog with better typography, and pruning it is the job, per requirements by union in `/euxlab-enterprise-ux-stance`.
...
Where the engagement is metric-driven, add two or three desired outcome statements in strict Ulwick format from `/euxlab-outcome-vocabulary`.
...
changes. /euxlab-design-review reads these labels.
