---
name: create-personas-and-job-stories
description: Read one research transcript into a professional enterprise persona - role, workflows, pain points, needs, and solution-free job stories, every trait cited. Run per transcript, before /synthesize, whenever an interview or contextual inquiry recording has been transcribed.
...
4. Load `/outcome-vocabulary` for the job story and outcome grammar, and `/evidence-standards` for grading.
...
One document per interviewee, two numbered sections, in this order. This format is the contract with `/synthesize`; do not reorder it and do not drop a section.
...
- **Desired outcomes:** two statements in strict Ulwick format from `/outcome-vocabulary`
...
Strict format from `/outcome-vocabulary`: *When [situation], I want to [motivation], so I can [outcome].*
...
- **The composite average.** A persona built from "what most people said" describes nobody. Build from this transcript; let `/synthesize` merge.
- **A story whose expected outcome is a feature.** "So I can use the new dashboard" is not an outcome.
- **An insight written here.** A pattern you notice in one transcript goes in as a cited pain point. `/synthesize` decides whether it is a pattern, and only `/synthesize` writes insights.
...
- The document is labeled `n=1, provisional` in its frontmatter until `/synthesize` reconciles it.
