...
2. Read `docs/design-brief.md` in full. If there is no design brief, stop and run `/design-brief` first. Reviewing against nothing produces opinions, and opinions are what this skill exists to avoid.
...
Load `/enterprise-ux-stance`. Check, in order:
...
4. **The ten enterprise smells** from `/enterprise-ux-stance`, each as a labeled judgment call: `Smell: <name>`, the evidence, the fix.
...
Load `/evidence-standards`. One question, asked of every screen: **which insight does this screen serve, and where is that written?**
