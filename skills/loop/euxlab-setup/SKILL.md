...
Per Constraint-Led Innovation in `/euxlab-enterprise-ux-stance`, this is the shape of the problem, not an obstacle. Record it as binding.
...
Who asks for the work, who signs off, and who is measured on the outcome. Names and roles. Three or four lines is enough; this exists so `/euxlab-grill-the-ask` knows whose assumptions it is grading and `/euxlab-design-brief` knows whose name goes on the confirmation.
...
Tell the user setup is complete and name the entry points: `/euxlab-grill-the-ask` when a stakeholder ask has arrived, `/euxlab-create-personas-and-job-stories` when a transcript has landed in `research/raw/`. If they ask what to open these files in: anything. They are markdown on their own disk. Mention that `docs/agents/engagement.md` is editable directly and that re-running this skill is only for switching clients or starting over.
