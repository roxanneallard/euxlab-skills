...
Read `docs/agents/engagement.md` first. If it does not exist, run `/euxlab-setup`.

Load `/euxlab-grilling` for the interview method and `/euxlab-evidence-standards` for the grading scale.

## Process

Run `/euxlab-grilling` against the ask. Same rounds, same frontier, same format, same rule that facts are your job and decisions are the user's.
...
8. **How would we know it worked?** If the answer is "adoption", press for a task outcome. See adoption theater in `/euxlab-enterprise-ux-stance`.
...
- **Ask about behavior, never about preference.** "What would you like to see?" produces a feature list and moves nothing off D. See the anti-patterns in `/euxlab-evidence-standards`.
