...
You invoke this by typing `/setup-euxlab-skills`. The agent won't reach for it on its own. In the Claude desktop and web apps, which have no slash commands, you name it in plain words instead, for example "set up EUXLab skills for this engagement".
...
For sharpening a stakeholder ask instead, use [grill-the-ask](https://euxlab.com/system/skills/grill-the-ask).
...
Setup records your design system, its version, and where its component list lives. No system yet becomes an explicit risk, not a blank. Per [enterprise-ux-stance](https://euxlab.com/system/skills/enterprise-ux-stance), the system is the shape of the problem, and [design-review](https://euxlab.com/system/skills/design-review) checks every screen against it.
...
- [grill-the-ask](https://euxlab.com/system/skills/grill-the-ask) comes next when a stakeholder ask arrives, because it reads the stakeholder map to know whose assumptions it grades.
- [create-personas-and-job-stories](https://euxlab.com/system/skills/create-personas-and-job-stories) is the other entry point when a transcript has already landed, because it reads the consent posture and ID scheme before it touches raw data.
