...
`/design-brief` turns your cited insights into the design assignment. It writes `docs/design-brief.md`: the scope, five to nine requirements, and the insight IDs each requirement serves.
...
You invoke this by typing `/design-brief`. The agent won't reach for it on its own. In the Claude desktop and web apps you name the skill in plain words instead, since those apps have no slash commands.

Reach for this when [/synthesize](https://euxlab.com/system/skills/synthesize) has finished and before anyone opens Figma.

For sharpening a raw stakeholder ask before any research instead, use [/grill-the-ask](https://euxlab.com/system/skills/grill-the-ask).
...
A screen that serves no listed insight is out of scope until the brief changes. Every output is the next input. The labelled design is what [/design-review](https://euxlab.com/system/skills/design-review) reads, and each screen label starts its trace chain.
...
- [/synthesize](https://euxlab.com/system/skills/synthesize) comes before, because the brief works from its insight index and cites its IDs.
- [/design-review](https://euxlab.com/system/skills/design-review) comes after the design, because it walks from each screen label to the insight and back to the requirement in this file. A requirement missing here cannot be checked later.
